...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    float rimBrighten = float(0.541);

    float rimWidth = lerp( - .05, 1, float(0.527));
    #ifdef POI_AUDIOLINK
    ...
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.191)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.191)) + lerp(albedo.rgb, rimColor, float(0.191)) * rimBrighten, rim);
}
#endif
